...
<div class="wrap-container clearfix">
    <div class="column-content-detail">
        ...
        <div class="layui-form" id="table-list">
                ...
                <c:forEach items="${pageInfo.list}" var="item">
                    <tr>
                       ...
                       <%--  <c:if test="${item.type==1}">--%>
                                <button class="layui-btn layui-btn-mini layui-btn-warm">未提交</button>
                         <%--   </c:if>
                            <c:if test="${item.type==2}">
                                <button class="layui-btn layui-btn-mini layui-btn-normal">已提交</button>
                            </c:if>--%>
                        </td>
                        <td>
                            <div class="layui-inline">
                                <a href="/toaddworkscha?rid=${item.rid}">提交作业</a>
                             <%--   <button title="提交" class="layui-btn layui-btn-small layui-btn-normal go-btn" data-id="1" data-url="/toaddworkscha?rid=${item.rid}"><i class="layui-icon">&#xe642;</i></button>
            ...
            <div class="page-wrap">
                <div>
                    <a href="/toSelectTest?title=${title}&pageNumStr=${pageInfo.firstPage}">首页</a>
                    <c:if test="${pageInfo.hasPreviousPage}">
                        <a href="/toSelectTest?title=${title}&pageNumStr=${pageInfo.prePage}">上一页</a>
                    </c:if>
                    <c:forEach items="${pageInfo.navigatepageNums}" var="i">
                        <a href="/toSelectTest?title=${title}&pageNumStr=${i}">${i}</a>
                    </c:forEach>
                    <c:if test="${pageInfo.hasNextPage}">
                        <a href="/toSelectTest?title=${title}&pageNumStr=${pageInfo.nextPage}">下一页</a>
                    </c:if>
                    <a href="/toSelectTest?title=${title}&pageNumStr=${pageInfo.lastPage}">尾页</a>
                </div>
            </div>
        </div>
    </div>
